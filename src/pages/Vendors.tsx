...
const Vendors = () => {
  ...
  const handleFollow = (vendorId: number) => {
    setVendors(vendors.map(vendor => 
      vendor.id === vendorId 
        ? { ...vendor, isFollowing: !vendor.isFollowing, followers: vendor.isFollowing ? vendor.followers - 1 : vendor.followers + 1 }
        : vendor
    ));
  };
      ...
      <div className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {vendors.map((vendor) => (
              <Card key={vendor.id} className={`${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white/70 border-blue-100'} backdrop-blur-lg hover:scale-105 transition-all duration-300 overflow-hidden`}>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-1 text-sm">
                          <MapPin className={`h-3 w-3 ${isDark ? 'text-slate-400' : 'text-slate-500'}`} />
                    ...
                    <Button
                      ...
                      className={vendor.isFollowing ? "" : "bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700"}
                    >
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    ...
                    <div className="text-center">
                      <div className="flex items-center justify-center space-x-1">
                        <ShoppingBag className={`h-4 w-4 ${isDark ? 'text-slate-300' : 'text-slate-600'}`} />
                        <span className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                    ...
                    <div className="text-center">
                      <div className="flex items-center justify-center space-x-1">
                        <Users className={`h-4 w-4 ${isDark ? 'text-slate-300' : 'text-slate-600'}`} />
                        <span className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                          {vendor.followers}
                        </span>
                      </div>
