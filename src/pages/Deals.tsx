...
const Deals = () => {
  ...
  const getBadgeColor = (badge: string) => {
    switch (badge) {
      ...
      case 'FLASH': return 'bg-yellow-500 text-black';
  ...
  const getBadgeIcon = (badge: string) => {
    switch (badge) {
      ...
      case 'LIMITED': return <Clock className="h-3 w-3" />;
  ...
  return (
    <div className={`min-h-screen transition-all duration-300 ${isDark ? 'dark bg-slate-900' : 'bg-gradient-to-br from-red-50 via-white to-orange-50'}`}>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-red-500 via-pink-500 to-purple-600 p-8 mb-12 text-center">
            <div className="relative z-10">
              <div className="flex items-center justify-center space-x-2 mb-4">
                ...
                <h1 className="text-4xl font-bold text-white">Flash Sale Event</h1>
                <Zap className="h-8 w-8 text-yellow-300" />
              </div>
              ...
              <div className="inline-flex items-center space-x-4 bg-white/20 backdrop-blur-sm rounded-full px-6 py-3">
                ...
                <span className="text-white font-semibold">Ends in 23:45:12</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {deals.map((deal) => {
              ...
              return (
                <Card key={deal.id} className={`${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white/70 border-orange-100'} backdrop-blur-lg hover:scale-105 transition-all duration-300 overflow-hidden group`}>
                  <div className="relative">
                    <div className={`aspect-square ${isDark ? 'bg-slate-700' : 'bg-orange-50'} flex items-center justify-center text-6xl group-hover:scale-110 transition-transform duration-300`}>
                      {deal.image}
                    </div>
                    ...
                    {/* Deal Badge */}
                    <Badge className={`absolute top-2 right-2 ${getBadgeColor(deal.badge)} text-white font-bold flex items-center space-x-1`}>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center space-x-2 mb-3">
                      ...
                      <span className={`text-lg line-through ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        ${deal.originalPrice}
                      </span>
                    </div>
